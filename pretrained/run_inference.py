...
from model import create_tiny_rmfa_net
from model import preprocess_raw_with_three_channel_split
from model import EfficientISP
from model import RobustISP
...
#INPUT_DIR = "../../ZRR2/test/huawei_raw"
...
class RawImageDataset_RMFA(Dataset):
    def __init__(self, image_dir):
        self.image_dir = image_dir
        self.image_files = [f for f in os.listdir(image_dir) if f.endswith(".png") or f.endswith(".jpg")]
    ...
    def __getitem__(self, idx):
        image_path = os.path.join(self.image_dir, self.image_files[idx])
        raw_ = Image.open(image_path)

        raw_ = np.array(raw_).astype(np.float32)
        raw_ = preprocess_raw_with_three_channel_split(torch.tensor(raw_, dtype=torch.float32))
        return raw_, self.image_files[idx]
...
MODEL_LOAD_PATH = f"models/Fujifilm/Efficient_ISP_unpaired_step_27220.pth"
#model = create_tiny_rmfa_net().to(device)
model = EfficientISP().to(device)
#model = RobustISP().to(device)
